...
{
	private static final Logger fabric_logger = new Logger("fabric");

	public static Mappings readFabricMapping(String path, String from, String to)
	{
		try
		{
			BufferedReader s = FileHandler.getReader(FileHandler.extract(FileHandler.download(path),"mappings/mappings.tiny"));
			fabric_logger.log("reading yarn mappings");
		...
		{
			fabric_logger.logError(e.getMessage());
		}
		return null;
	}
}
